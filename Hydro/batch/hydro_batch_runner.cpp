...
fs::path findRepositoryRoot(const fs::path& start) {
    ...
    while (!current.empty()) {
        if (fs::exists(current / "HydroPINN.pro") ||
            fs::exists(current / "HydroBatch.pro")) {
            return current;
        }
    ...
    throw std::runtime_error(
        "Unable to locate PyTorchCPP repository root from: " + start.string());
}
...
void resolveConfigPaths(HydroRunConfig& config, const fs::path& repository_root) {
    if (config.use_hydro_package && !config.hydro_package_path.empty()) {
        fs::path package_path(config.hydro_package_path);
        if (package_path.is_relative()) {
            package_path = repository_root / package_path;
        }
        config.hydro_package_path = fs::weakly_canonical(package_path).string();
    }

    if (config.use_csv_data && !config.csv_path.empty()) {
        fs::path csv_path(config.csv_path);
        if (csv_path.is_relative()) {
            csv_path = repository_root / csv_path;
        }
        config.csv_path = fs::weakly_canonical(csv_path).string();
    }
}
        ...
        for (const auto& job : jobs) {
            ...
            try {
                ...
                if (config.use_hydro_package) {
                    std::cout << "[batch] hydro_package_path=" << config.hydro_package_path << '\n';
                }

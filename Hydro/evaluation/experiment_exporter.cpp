...
void HydroExperimentExporter::exportRun(const std::string& outputDirectory,
    ...
    metrics << "approach,success,final_loss,validation_mse,test_mse,rmse,mae,nse,kge,correlation,pbias,volume_error_percent,peak_timing_error,peak_magnitude_error_percent,physics_loss\n";
    ...
    for (const auto& entry : results) {
        ...
        metrics << entry.first << ',' << (r.success ? 1 : 0) << ',' << r.final_loss << ',' << r.validation_mse << ','
                ...
                << r.peak_magnitude_error_percent << ',' << r.physics_loss << '\n';
    }

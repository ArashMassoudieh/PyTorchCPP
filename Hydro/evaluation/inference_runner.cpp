...
public:
    TemporaryCheckpoint(const std::vector<std::uint8_t>& bytes, const std::string& prefix)
        : path_(temporaryHydroCheckpointPath(prefix)) {
        std::ofstream output(path_, std::ios::binary);
        if (!output) {
            std::filesystem::remove(path_);
            throw std::runtime_error("Unable to materialize inference checkpoint.");
        }
        output.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
        if (!output) {
            output.close();
            std::filesystem::remove(path_);
            throw std::runtime_error("Unable to write inference checkpoint.");
        }
    }

    ~TemporaryCheckpoint() { std::error_code error; std::filesystem::remove(path_, error); }
...
torch::Tensor HydroInferenceRunner::predictFeedForward(
    ...
    model.loadModel(checkpoint.path().string());

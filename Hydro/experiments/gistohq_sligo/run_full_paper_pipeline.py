...
selected Hydro package or CSV source. It finally generates split-shift and
rolling-origin hydrologic diagnostics, metric-definition diagnostics, combined
paper tables, a post-hoc hybrid-gain assessment, frozen configs, and publication
figures. Real-data selection uses validation data only; held-out test metrics
are never used to choose hyperparameters.
"""
...
ROLLING_DIAGNOSTICS = HERE / "diagnose_rolling_folds.py"
...
def run(cmd: list[str]) -> None:
    ...
    process = subprocess.Popen(
        [str(v) for v in cmd], cwd=HERE, stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT, text=True, bufsize=1,
    )
    assert process.stdout is not None
    for line in process.stdout:
        sys.stdout.write(line); sys.stdout.flush(); append_log(line.rstrip("\n"))
    code = process.wait()
    if code != 0:
...
def real_source_args(a: argparse.Namespace) -> list[str]:
    return ["--data-source", a.data_source, "--csv-path", a.csv_path,
            "--csv-x-column", str(a.csv_x_column), "--csv-y-column", str(a.csv_y_column),
            "--csv-has-header", a.csv_has_header, "--hydro-package-path", a.hydro_package_path,
            "--hydro-catchment-id", a.hydro_catchment_id,
            "--hydro-package-profile", a.hydro_package_profile, "--epochs", str(a.epochs),
...
def main() -> int:
    global LOG_PATH
    a = parser().parse_args()
    root = a.output_root.resolve(); root.mkdir(parents=True, exist_ok=True)
    LOG_PATH = root / "full_pipeline.log"
    LOG_PATH.write_text("HydroPINN full paper pipeline log\n" +
    ...
    if a.data_source == "hydro" and not a.hydro_package_path.strip(): raise SystemExit("Hydro package path is required")
    if a.data_source == "csv" and not a.csv_path.strip(): raise SystemExit("CSV path is required")

    metadata = root / "paper_run_metadata.txt"
    metadata.write_text(
        "started_utc=" + datetime.now(timezone.utc).isoformat() + "\n" +
        f"hydrobatch={a.hydrobatch.resolve()}\nreal_data_source={a.data_source}\n" +
        f"hydro_package_path={a.hydro_package_path}\ncsv_path={a.csv_path}\nfull_pipeline_log={LOG_PATH}\n" +
        ...
        "rolling_fold_diagnostics=expanding chronological folds; diagnostic only\n" +
        f"process_hybrid_preflight={'skipped' if a.skip_process_preflight else 'required'}\n",
        encoding="utf-8")

    preflight = root / "00_process_hybrid_preflight"; synthetic = root / "01_synthetic_controlled"; real = root / "02_sligo_hydro"
    say("\n[full-paper] 0a/4 GUI routing + synthetic-physics regression checks")
    run([sys.executable, REGRESSION_CHECKS])
    with metadata.open("a", encoding="utf-8") as f: f.write("gui_physics_regression_status=pass\n")

    if not a.skip_process_preflight:
        say("\n[full-paper] 0b/4 Process-aware LSTM+PINN preflight")
        run([sys.executable, PREFLIGHT, "--hydrobatch", a.hydrobatch.resolve(), "--output-root", preflight,
             "--epochs", "20", *preflight_source_args(a)])
        with metadata.open("a", encoding="utf-8") as f: f.write("process_hybrid_preflight_status=pass\n")

    say("\n[full-paper] 1/4 Controlled reduced-reservoir verification")
    run([sys.executable, ADAPTIVE, "--hydrobatch", a.hydrobatch.resolve(), "--output-root", synthetic,
         "--data-source", "synthetic", "--synthetic-profile", "reduced_reservoir",
         "--synthetic-truth-k", str(a.synthetic_truth_k), "--sample-count", str(a.synthetic_sample_count),
         "--t-start", str(a.synthetic_t_start), "--t-end", str(a.synthetic_t_end), "--epochs", str(a.epochs),
         "--robust-seeds", a.robust_seeds])

    say("\n[full-paper] 2/4 Real-data five-method adaptive study")
    run([sys.executable, ADAPTIVE, "--hydrobatch", a.hydrobatch.resolve(), "--output-root", real, *real_source_args(a)])

    say("\n[full-paper] 3/4 Hydrologic diagnostics, metric definitions, final tables, and hybrid assessment")
    run([sys.executable, SPLIT_DIAGNOSTICS, real])
    run([sys.executable, ROLLING_DIAGNOSTICS, real])
    run([sys.executable, POSTPROCESS, synthetic]); run([sys.executable, POSTPROCESS, real])
    run([sys.executable, TABLES, root]); run([sys.executable, ASSESS, root])
    ...
    say("\n[full-paper] COMPLETE")
    say("[full-paper] output: " + str(root)); say("[full-paper] log: " + str(LOG_PATH))
    ...
    say("[full-paper] rolling diagnostics: " + str(real / "paper_rolling_fold_diagnostics.csv"))
...
if __name__ == "__main__":
    try: raise SystemExit(main())
        ...
        append_log(f"[full-paper] FAILED: {type(exc).__name__}: {exc}"); raise
